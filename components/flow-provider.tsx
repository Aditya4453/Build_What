...
export function FlowProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(()=>{fetch("/api/me").then(r=>r.ok?r.json():null).then(data=>{if(data?.user){setCitizen(data.user);if(data.context?.payment?.status)setPaymentStatus(data.context.payment.status)}}).catch(()=>undefined)},[]);
